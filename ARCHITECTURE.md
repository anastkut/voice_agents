...
clients — an invalidation ping, never a payload. Phone numbers are normalized
digits-only on create and lookup. Audit events are written in the same transaction
...
```
cases        id, created_at, updated_at, status, urgency, name, phone,
...
- **Caller** (via agent tools): create a case, look up / note / re-prioritize /
  cancel their own — after verifying name and case description; a case number
  alone discloses nothing. Verification is prompt-level here; in production the
  comparison moves inside the tool so unverified data never enters the LLM context.
